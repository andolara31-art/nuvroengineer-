export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="py-12 bg-brand-black/40 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex flex-col items-center md:items-start gap-2">
          <a href="#" className="text-xl font-display font-bold tracking-tight text-brand-white">
            NUVRO<span className="text-brand-platinum/50 font-light"> ENGINEER</span>
          </a>
          <p className="text-sm text-brand-platinum/50 uppercase tracking-widest font-mono text-[10px]">
            Microservicios digitales y proyectos custom.
          </p>
        </div>
        
        <nav className="flex flex-wrap items-center justify-center gap-6">
          <a href="#express" className="text-sm font-medium text-brand-platinum/70 hover:text-brand-white transition-colors">Express</a>
          <a href="#paquetes" className="text-sm font-medium text-brand-platinum/70 hover:text-brand-white transition-colors">Paquetes</a>
          <a href="#custom" className="text-sm font-medium text-brand-platinum/70 hover:text-brand-white transition-colors">Custom</a>
          <a href="#proyectos" className="text-sm font-medium text-brand-platinum/70 hover:text-brand-white transition-colors">Proyectos</a>
          <a href="#contacto" className="text-sm font-medium text-brand-platinum/70 hover:text-brand-white transition-colors">Contacto</a>
        </nav>
        
        <div className="text-sm text-brand-platinum/50">
          &copy; {currentYear} NUVRO ENGINEER. Todos los derechos reservados.
        </div>
      </div>
    </footer>
  );
}
